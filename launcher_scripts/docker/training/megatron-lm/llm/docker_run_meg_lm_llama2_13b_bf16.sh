...
for host in `cat ${HOSTFILE}`;do
    ssh root@$host "docker run -d --name=llama2-13b-test \
                    ...
                    bash -c \"git clone --recursive https://github.com/sallylxl/deep_learning_examples.git -b feature/megatron-lm-patch ${DEEP_LEARNING_EXAMPLES_DIR} || \
                              ...
                              MOCK_DATA=true RUN_ID=${TIME} DEEP_LEARNING_EXAMPLES_DIR=${DEEP_LEARNING_EXAMPLES_DIR} TIMING_LOG_LEVEL=2 GBS=256 \
                              bash ${DEEP_LEARNING_EXAMPLES_DIR}/training/Megatron-LM/llm/llama/run_meg_lm_llama2_13b_bf16.sh \""
    ((i++))
done
